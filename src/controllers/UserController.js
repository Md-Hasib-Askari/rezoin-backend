...
exports.register = async function (req, res) {
  try {
    ...
    if (cookieEmail) {
      return res.status(200).json({status: "success", message: "Already Logged In" });
    } else {
      // Hash the password
...
exports.userLogin = async (req, res) => {
  try {
    ...
    if (!user) {
      ...
      // Generate JWT token
      const token = jwt.sign({ email }, jwt_token, { expiresIn: "1h" });

      // Set token and email in cookies
      res.cookie("token", token, {httpOnly: true});
      res.cookie("email", email, {httpOnly: true});
      res.status(200).json({status: "success", message: "Login Successful!", token, email });
    }
  } catch (error) {
